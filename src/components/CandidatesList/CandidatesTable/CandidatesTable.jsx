...
export default function CandidatesTable(props) {
    ...
    const {
        candidates, page, setPage, rowsPerPage, setRowsPerPage,
        searchTerm, setSearchTerm, totalCount
    } = props;

    const emptyRows = Math.max(0, rowsPerPage - candidates.length);
        ...
        <div>
            ...
            <TableContainer component={Paper} sx={{ maxHeight: 550, overflowY: 'auto' }}>
                <Table stickyHeader sx={{ minWidth: 500 }} aria-label="custom pagination table">
                    ...
                    <TableBody>
                        {/* Map directly over candidates. No .slice() needed since backend did it! */}
                        {candidates.map((row) => (
                            <TableRow
                                key={row.id}
                                onMouseOver={(e) => (e.currentTarget.style.backgroundColor = '#f0f0f0')}
                                onMouseOut={(e) => (e.currentTarget.style.backgroundColor = 'transparent')}
                                onClick={() => props.setSelectedCandidate(row.id)}
                            >
                                <TableCell sx={{ width: '15%' }}>{row.firstName}</TableCell>
                                <TableCell sx={{ width: '15%' }} align="right">{row.lastName}</TableCell>
                                <TableCell sx={{ width: '10%' }} align="right">{row.experience}</TableCell>
                                <TableCell sx={{ width: '45%' }} align="right">{row.position}</TableCell>
                        ...
                        <TableRow>
                            <TablePagination
                                sx={{
                                    ...
                                    borderTop: '1px solid', borderColor: 'divider', left: 0, right: 0,
                                }}
                                ...
                                page={page}
                                slotProps={{
                                    select: { inputProps: { 'aria-label': 'Candidates per page' }, native: true },
                                }}
